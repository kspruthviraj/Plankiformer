...
class create_and_train_model:
    ...
    def create_data_loaders(self, fplankton, class_main):
    ...
    def import_deit_models(self, fplankton, class_main):
        ...
        self.criterion = nn.CrossEntropyLoss(fplankton.class_weights)
    ...
    def run_training(self, class_main):
        ...
        for epoch in range(class_main.params.epochs):
            ...
            train_acc1, train_loss, train_outputs, train_targets = cls_train(self.train_dataloader, self.model, self.criterion,
                                                                             self.optimizer,
                                                                             class_main.params.clip_grad_norm)
            acc1, loss, test_outputs, test_targets, total_mins = cls_validate(self.val_dataloader, self.model, self.criterion
            ...
            if test_f1 > best_f1:
                torch.save({'model_state_dict': self.model.state_dict(),
                            'optimizer_state_dict': self.optimizer.state_dict()}, self.checkpoint_path + '/trained_model.pth')
    ...
    def train_and_save(self, fplankton, class_main):
        self.create_data_loaders(self, fplankton, class_main)
        self.import_deit_models(self, fplankton, class_main)
        self.run_training(self, class_main)
