...
class Cdata_others:
    ...
    def __init__(self, datapath, testpath, L=None, class_select=None, classifier=None, compute_extrafeat=None,
                 resize_images=None, balance_weight=None, kind='mixed', training_data=True):
        self.X_test = None
        self.Ximage_test = None
        self.Xfeat_test = None
        self.test_filenames = None
        self.Xfeat_train = None
        self.Ximage_train = None
        self.X_train = None
        ...
        self.y = None
        self.X = None
        self.Load_others(self.datapath, self.testpath,  self.L, self.class_select, self.classifier,
                         self.compute_extrafeat, self.resize_images, training_data=training_data)
        return

    def Load_others(self, datapaths, testpaths, L, class_select, classifier, compute_extrafeat, resize_images,
                    kind='mixed', training_data=True):
        """
        Loads dataset
        For the moment, only mixed data. Later, also pure images or pure features.
        ...
        if kind == 'image':
            self.df_train = LoadImages(datapaths, L, resize_images, training_data=training_data)
            if compute_extrafeat == 'yes':
                dfExtraFeat_train = compute_extrafeat_function(self.df_train)
                self.df_train = pd.concat([self.df_train, dfExtraFeat_train], axis=1)

            self.df_test = LoadImages(testpaths, L, resize_images, training_data=training_data)
            if compute_extrafeat == 'yes':
                dfExtraFeat_test = compute_extrafeat_function(self.df_test)
                self.df_test = pd.concat([self.df_test, dfExtraFeat_test], axis=1)
        else:
            raise NotImplementedError('Please choose kind as image')

        self.kind = kind
        self.CreateXy_others()  # Creates X and y, i.e. features and labels
        return

    def CreateXy_others(self):
        """
        Creates features and target
        - removing the evidently junk columns.
        - allowing to access images and features separately and confortably
        ...
        self.train_filenames = self.df_train.filename
        self.X_train = self.df_train.drop(columns=['classname', 'url', 'filename', 'file_size', 'timestamp'], errors='ignore')
        self.Ximage_train = self.X_train.npimage if (self.kind != 'feat') else None
        self.Xfeat_train = self.X_train.drop(columns=['npimage'], errors='ignore') if (self.kind != 'image') else None

        self.test_filenames = self.df_test.filename
        self.X_test = self.df_test.drop(columns=['classname', 'url', 'filename', 'file_size', 'timestamp'], errors='ignore')
        self.Ximage_test = self.X_test.npimage if (self.kind != 'feat') else None
        self.Xfeat_test = self.X_test.drop(columns=['npimage'], errors='ignore') if (self.kind != 'image') else None

        return


class Data_array_for_others:
    """
    A class for extracting train and test sets from the original dataset, and preprocessing them.
    """

    def __init__(self, X_train, y_train, X_test, y_test, train_filenames, test_filenames, ttkind='image',
                 classifier=None, balance_weight=None, testSplit=0.2, valid_set=None, test_set=None,
                 compute_extrafeat=None, random_state=12345):
        """
        X and y are dataframes with features and labels
        """
        ...
        # Now the features
        self.X_train = self.ImageNumpyFromMixedDataframe(X_train)
        self.X_test = self.ImageNumpyFromMixedDataframe(X_test)

        self.trainXimage = self.ImageNumpyFromMixedDataframe(self.X_train)
        self.testXimage = self.ImageNumpyFromMixedDataframe(self.X_test)
